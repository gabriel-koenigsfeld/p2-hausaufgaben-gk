...
public class Game implements ITickableListener, IKeyboardListener
{
	...
	public void init()
	{
		...
		frameWork.setBackground(new Background("01_hausaufgabe\\assets\\sprite_background.png"));
		frameWork.setBackgroundColor(new Color(0, 0, 0));
		
		ship = new Ship((screenWidth / 2) - 30, 4 * screenHeight / 5, screenWidth/20, screenWidth/20, "01_hausaufgabe\\assets\\sprite_ship.png");
		frameWork.addGameObject(ship);
		
		Ufo ufo = new Ufo(-20, screenHeight/5, screenWidth/10, screenWidth/10, 9, "01_hausaufgabe\\assets\\sprite_ufo.png");
		ufos.add(ufo);
	...
	{
		if(shipAmmo > 0) {
			Projectile projectile = new Projectile(ship.getX()+11, ship.getY()+5, ship.getWidth()/2, ship.getWidth()/2, 60, "01_hausaufgabe\\assets\\sprite_projectile2.png");
